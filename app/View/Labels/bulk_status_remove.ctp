...
<?php foreach ($labels as $k => $label): ?>
                <tr>
                    ...
                    <td><?= $this->Time->format('d/m/y h:i:s a', $label['Label']['print_date'], 'invalid'); ?></td>
                    <td><?= h($label['Location']['location']); ?></td>

                    <td><?= h($label['InventoryStatus']['name']); ?></td>
                    <td><?= h($label['Label']['inventory_status_note']); ?></td>
    ...
    <?php foreach ($status_list as $key => $value): ?>
                            <label class="radio-inline">
                                <input type="radio" data-checked="0" name="data[Label][<?= $k; ?>][inventory_status_id]" value="<?= $key; ?>"><?= $value; ?>
                            </label>
    <?php endforeach; ?>
