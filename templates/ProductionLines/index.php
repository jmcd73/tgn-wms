...
<table class="table table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?= $productionLine->has('printer') ? $this->Html->link($productionLine->printer->name, ['controller' => 'Printers', 'action' => 'view', $productionLine->printer->id]) : '' ?></td>
            <td><?= h($productionLine->name) ?></td>
            <td><?= $productionLine->has('product_type') ? $this->Html->link($productionLine->product_type->name, ['controller' => 'ProductTypes', 'action' => 'view', $productionLine->product_type->id]) : '' ?></td>
            <td class="actions">
                <?= $this->Html->link(__('View'), ['action' => 'view', $productionLine->id], ['title' => __('View'), 'class' => 'btn btn-secondary']) ?>
                <?= $this->Html->link(__('Edit'), ['action' => 'edit', $productionLine->id], ['title' => __('Edit'), 'class' => 'btn btn-secondary']) ?>
                <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $productionLine->id], ['confirm' => __('Are you sure you want to delete # {0}?', $productionLine->id), 'title' => __('Delete'), 'class' => 'btn btn-danger']) ?>
            </td>
        </tr>
        <?php endforeach; ?>
    </tbody>
